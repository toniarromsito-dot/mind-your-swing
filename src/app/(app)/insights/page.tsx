...
import { LineChart, ChevronRight, TrendingUp, TrendingDown } from "lucide-react";
...
import { computeMentalScore, mentalStateFromScore, MOOD_SCORE } from "@/lib/mood";
import {
  getMentalTrendData,
...
export default async function InsightsPage() {
  ...
  const [mentalEntries, recoveryEvents, pressureData, current30, previous30, roundScoreTrend] = await Promise.all([
  ...
  const mentalChartData = [...mentalEntries]
    .reverse()
    .map((e, i) => ({ label: `#${i + 1}`, value: Math.round(((MOOD_SCORE[e.mood] + 2) / 4) * 100) }));
  ...
  const onCourseChartData = roundScoreTrend.map((p) => ({
    label: p.date.toLocaleDateString(t.dateLocale, { day: "numeric", month: "short" }),
    value: p.relative,
  }));
  const onCourseDomain: [number, number] =
    onCourseChartData.length > 0
      ? [Math.min(0, ...onCourseChartData.map((p) => p.value)) - 2, Math.max(0, ...onCourseChartData.map((p) => p.value)) + 2]
      : [0, 10];
  ...
  ].filter((c): c is { key: string; category: string; photo: string; text: string } => Boolean(c));
    ...
    <div className="mx-auto flex max-w-2xl flex-col gap-8">
      ...
      <div className="flex flex-col gap-3">
        ...
        <Tabs defaultValue="mental">
          <TabsList>
            <TabsTrigger value="mental">{t.insights.trendTabMental}</TabsTrigger>
            <TabsTrigger value="onCourse">{t.insights.trendTabOnCourse}</TabsTrigger>
            <TabsTrigger value="practice">{t.insights.trendTabPractice}</TabsTrigger>
            <TabsTrigger value="wellbeing">{t.insights.trendTabWellbeing}</TabsTrigger>
          </TabsList>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <Card className="flex-1 border-border/70 shadow-none">
                <CardContent className="py-4">
                  <TrendChart data={mentalChartData} domain={[0, 100]} emptyLabel={t.insights.emptyChart} />
                </CardContent>
              </Card>
              {percentChange && (
                <Card className="border-border/70 shadow-none sm:w-44 sm:shrink-0">
                  <CardContent className="flex flex-col gap-1 py-4">
                    ...
                    <p className="text-xs font-medium">{t.insights.trendsScoreLabel}</p>
                    <p className="text-xs text-muted-foreground">{t.insights.vsLastPeriod}</p>
                    <p className="mt-2 flex items-center gap-1.5 text-xs text-primary">
                      ...
                      {percentChange.trend === "up" ? t.insights.positiveTrendLabel : t.insights.negativeTrendLabel}
                    </p>
                  </CardContent>
                </Card>
              )}
          ...
          <TabsContent value="onCourse" className="mt-4">
            <Card className="border-border/70 shadow-none">
              <CardContent className="py-4">
                <TrendChart data={onCourseChartData} domain={onCourseDomain} emptyLabel={t.insights.emptyOnCourseTab} />
              </CardContent>
            </Card>
          </TabsContent>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="-mx-4 flex gap-3 overflow-x-auto px-4 pb-1 sm:mx-0 sm:px-0">
            {keyInsights.map((insight) => (
              <div key={insight.key} className="relative h-44 w-40 shrink-0 overflow-hidden rounded-2xl">
                <Image src={insight.photo} alt="" fill sizes="160px" className="object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/15 to-transparent" />
                <div className="absolute inset-x-0 bottom-0 p-3">
                  <p className="text-sm leading-tight font-medium text-white">{insight.text}</p>
      ...
      <div className="flex flex-col gap-3">
        ...
        >
          <div className="relative size-16 shrink-0 overflow-hidden rounded-xl">
            <Image src="/images/coach-topic-ball.jpg" alt="" fill sizes="64px" className="object-cover" />
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-semibold tracking-[0.1em] text-muted-foreground uppercase">
              {t.coach.categoryLabels.mental}
            </p>
            <p className="font-heading text-base font-semibold">{t.preShotRoutine.title}</p>
            <p className="mt-0.5 text-xs text-muted-foreground">{t.insights.recommendationRoutineDescription}</p>
          </div>
