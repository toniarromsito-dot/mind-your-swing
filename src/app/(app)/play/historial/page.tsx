...
export default async function PlayHistoryPage() {
  ...
  function GameRow({ game }: { game: (typeof games)[number] }) {
    ...
    const me = standings.find(
      (s) => game.players.find((p) => p.id === s.playerId)?.user.id === userId
    );
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                ...
                {game.players.length > 1 &&
                  ` · ${game.players.length} ${t.play.players}`}
              </p>
            </div>
            <div className="flex items-center gap-3">
              {game.status === "IN_PROGRESS" && (
                <Badge variant="secondary">{t.play.inProgress}</Badge>
              )}
              {me && (
                <span className="font-heading text-lg">{me.relativeToPar}</span>
              )}
            </div>
          </CardContent>
        </Card>
